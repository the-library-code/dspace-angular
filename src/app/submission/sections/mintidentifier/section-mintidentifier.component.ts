...
import { SECTION_LICENSE_FORM_LAYOUT, SECTION_LICENSE_FORM_MODEL } from './section-mintidentifier.model';
...
@Component({
  ...
  styleUrls: ['./section-mintidentifier.component.scss'],
  templateUrl: './section-mintidentifier.component.html',
})
@renderSectionFor(SectionsType.License)
export class SubmissionSectionMintIdentifierComponent extends SectionModelComponent {
   ...
   */
  public formLayout: DynamicFormLayout = SECTION_MINTIDENTIFIER_FORM_LAYOUT;
  ...
  /**
   * Initialize all instance variables and retrieve identifier data
   */
  onSectionInit() {
    this.pathCombiner = new JsonPatchOperationPathCombiner('sections', this.sectionData.id);
    this.formId = this.formService.getUniqueId(this.sectionData.id);
    this.formModel = this.formBuilderService.fromJSON(SECTION_MINTIDENTIFIER_FORM_MODEL);
    const model = this.formBuilderService.findById('granted', this.formModel);

    // Retrieve license accepted status
    (model as DynamicCheckboxModel).value = (this.sectionData.data as WorkspaceitemSectionIdentifiersObject).granted;
